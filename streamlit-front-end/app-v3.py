...
def get_pulled_models(base_url=None):
    base_url = base_url or default_base_url
    ok, data = check_ollama_health(base_url)
    if not ok:
        return [], data  # return empty list and the error message
    # Try to parse a variety of possible shapes:
    if isinstance(data, dict):
        # Ollama often returns {"models": [{"name": "llama3.1:8b", ...}, ...]}
        models = data.get("models") or data.get("tags") or []
        return [m.get("name") if isinstance(m, dict) else str(m) for m in models], data
    # fallback if JSON is just a list of names
